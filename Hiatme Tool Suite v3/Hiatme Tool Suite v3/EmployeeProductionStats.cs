...
namespace Hiatme_Tool_Suite_v3
{
    internal class EmployeeProductionStats
    {
        ...
        private int CheckIfDriversTimesAreAccurate(WRDownloadedTrip driverwrtrip, MCDownloadedTrip mctrip)
        {
            ...
            DateTime schedputime = DateTime.ParseExact(mctrip.PUTime, "HH:mm", CultureInfo.InvariantCulture);
            DateTime scheddotime = DateTime.ParseExact(mctrip.DOTime, "HH:mm", CultureInfo.InvariantCulture);
            DateTime driverputime = DateTime.ParseExact(driverwrtrip.ActualPUTime, "HH:mm", CultureInfo.InvariantCulture);
            DateTime driverdotime = DateTime.ParseExact(driverwrtrip.ActualDOTime, "HH:mm", CultureInfo.InvariantCulture);
                ...
                if (wrtrip.Status == "Completed" || wrtrip.Status == "Billed")
                {
                    foreach (MCDownloadedTrip mctrip in mcdledtriplist)
                    {
                        if (mctrip.TripNumber == wrtrip.TripNumber)
                        {
                            ...
                            accuraciescounter = accuraciescounter + CheckIfDriversTimesAreAccurate(wrtrip, mctrip);
                            Console.WriteLine(mctrip.TripNumber + ": " + CheckIfDriversTimesAreAccurate(wrtrip,mctrip).ToString());
                        }
                    }
                }
            }
        ...
        private void GenerateWorkloadStat(List<WRDownloadedTrip> wrdtlist, int numofworkers)
        {
            ...
            double fairsharepercent = 100 / numofworkers;
